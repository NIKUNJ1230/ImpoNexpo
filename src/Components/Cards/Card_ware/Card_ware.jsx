import React, { useEffect, useRef, useState } from "react";
import "./Card_ware.css";
import { Link } from "react-router-dom";
import PersonAddIcon from "@mui/icons-material/PersonAdd";
import shoes from "../../../assets/images/live video.svg"
import { ReactComponent as Play } from "../../../assets/icons/play.svg";
import Comment from "../../../assets/icons/comment.svg";
import Share from "../../../assets/icons/share.svg";
import Download from "../../../assets/icons/download.svg";
import ReactPlayer from "react-player";
import Flag from "react-world-flags";
import FavoriteBorderRoundedIcon from "@mui/icons-material/FavoriteBorderRounded";
import FavoriteRoundedIcon from "@mui/icons-material/FavoriteRounded";
import ShoppingCartIcon from "@mui/icons-material/ShoppingCart";
import Diversity3Icon from "@mui/icons-material/Diversity3";
import MarkUnreadChatAltRoundedIcon from "@mui/icons-material/MarkUnreadChatAltRounded";
const Card_ware = () => {
  return (
    <div className="card_ware__component_wrap">
      <div className="card_ware__component_header">
        <Link to="#" className="card_ware__component_hdrleft">
          <div className="card_ware__component_hlimgwrap">
            <img
              src="data:image/jpeg;base64,"
              className="card_ware__component_hlimg"
            />
          </div>
          <div className="card_ware__component_hlcontent">
            <div className="card_ware__component_hlcname">
              David Semilons
              <Flag
                code="NG"
                fallback={<span>{""}</span>}
                style={{
                  height: "10px",
                  width: "15px",
                  marginLeft: "5px",
                }}
              />
            </div>
            <div className="card_ware__component_hlchandle">@davidsem22</div>
          </div>
        </Link>
        <div className="card_ware__component_hdrcenter"></div>
        <div className="card_ware__component_hdrright">
          <div className="card_ware__component_hrfollowbtn">
            <PersonAddIcon className="card_ware__component_hrfollowbtnicn" />
            <span className="card_ware__component_hrfollowbtntxt">Follow</span>
          </div>
          <div className="card_ware__component_hrfollowcount">120 followers</div>
        </div>
      </div>
      <div className="card_ware__component_media">
      <img src={shoes} alt="" />
  
      </div>
      <div className="card_ware__component_stats">
        <div className="card_ware__component_statitem">
          <FavoriteBorderRoundedIcon className="card_ware__component_statitemicon" />
          {/* <FavoriteRoundedIcon className="card_ware__component_statitemicon" /> */}
          <div className="card_ware__component_statitemtext">231</div>
        </div>
        <div className="card_ware__component_statitem">
          {/* <Comment className="card_ware__component_statitemicon" /> */}
          <img src={Comment} alt="" className="card_ware__component_statitemicon" />
          {/* <FavoriteIcon className="card_ware__component_statitemicon" /> */}
          <div className="card_ware__component_statitemtext">450</div>
        </div>
        <div className="card_ware__component_statitem">
          <img src={Share} alt="" className="card_ware__component_statitemicon" />
          {/* <FavoriteIcon className="card_ware__component_statitemicon" /> */}
          <div className="card_ware__component_statitemtext">34</div>
        </div>
        <div className="card_ware__component_statitem">
          <img
            src={Download}
            alt=""
            className="card_ware__component_statitemicon"
          />
          {/* <FavoriteIcon className="card_ware__component_statitemicon" /> */}
          <div className="card_ware__component_statitemtext">500</div>
        </div>
      </div>
      <Link to="#" className="card_ware__component_title">
        Portable No Lace Safety Boots
      </Link>
      <div className="card_ware__component_tags">
        <div className="card_ware__component_tag">#go</div>
        <div className="card_ware__component_tag">#World</div>
        <div className="card_ware__component_tag">#David</div>
        <div className="card_ware__component_tag">#Semllons</div>
        <div className="card_ware__component_tag">#Export</div>
      </div>
      <div className="card_ware__component_desc">
        Lorem ipsum dolor sit amet consectetur. Sit dis fusce quis etiam arcu.
        Parturient sagittis facilisi nunc viverra non malesuada{" "}
        <Link to="#" className="card_ware__component_descreadmore">
          Read More.
        </Link>
        .
      </div>
      <div className="card_ware__component_pricesec">
        <div className="card_ware__component_priceleft">$4.45- $6.5</div>
        <div className="card_ware__component_priceright">Min orders: 20 Units</div>
      </div>
      <div className="card_ware__components_footer">
        <div className="card_ware__components_fbtn">
          <ShoppingCartIcon className="card_ware__components_fbtnicn" />
          Order Now
        </div>
        <div className="card_ware__components_fbtn">
          <Diversity3Icon className="card_ware__components_fbtnicn" />
          Group Import
        </div>
        <div className="card_ware__components_fbtn">
          <MarkUnreadChatAltRoundedIcon className="card_ware__components_fbtnicn" />
          Contact Exporter
        </div>
      </div>
    </div>
  );
};

export default Card_ware;
