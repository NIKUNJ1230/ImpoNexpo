import React from 'react'
import { Link } from 'react-router-dom'
import Flag from "react-world-flags";
import PersonAddIcon from "@mui/icons-material/PersonAdd";
import { IoPlaySharp } from "react-icons/io5";
function Single_trending_suppliers_live({ profile, name, acc, flag, isFollow, followers }) {
    return (
        <div className="w-5">
            <div className="single-trending-suppliers-live">
                <div className="trendsuppli__live__component_header">
                    <Link to="/" className="trendsuppli__live__component_hdrleft">
                        <div className="trendsuppli__live__component_hlimgwrap">
                            <img
                                src="data:image/jpeg;base64,"
                                className="trendsuppli__live__component_hlimg"
                            />
                        </div>
                        <div className="trendsuppli__live__component_hlcontent">
                            <div className="trendsuppli__live__component_hlcname">
                                {name}
                                <Flag
                                    code={flag}
                                    fallback={<span>{""}</span>}
                                    style={{
                                        height: "10px",
                                        width: "15px",
                                        marginLeft: "5px",
                                    }}
                                />
                            </div>
                            <div className="trendsuppli__live__component_hlchandle">{acc}</div>
                        </div>
                    </Link>
                    <div className="trendsuppli__live__component_hdrcenter"></div>
                    <div className="trendsuppli__live__component_hdrright">
                        <div className="trendsuppli__live__component_hrfollowbtn">
                            <PersonAddIcon className="trendsuppli__live__component_hrfollowbtnicn" />
                            <span className="trendsuppli__live__component_hrfollowbtntxt">{isFollow}</span>
                        </div>
                        <div className="trendsuppli__live__component_hrfollowcount">{followers} followers</div>
                    </div>
                </div>
                <div className="trendsuppli_play-btn">
                    <Link to="live_stream_main">
                        <IoPlaySharp />
                    </Link>
                </div>
            </div>
        </div>
    )
}

export default Single_trending_suppliers_live
